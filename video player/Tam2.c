...
typedef struct VideoState {
	...
	struct SwsContext *sws_ctxRGB;	/* struct that built for changing colors for RGB	*/
...
SDL_Surface *screen;
VideoState *nChannel[3];		//global pointer for our main structer(Channel)
int AudioChan=0;						//global var for the audio station
int Channel=0;							////global var for the video station
SDL_mutex *cc;
...
void audio_callback(void *userdata, Uint8 *stream, int len){
			...
			VideoState *is = nChannel[AudioChan];
			int len1, audio_size;
			double pts;
...
int queue_picture(VideoState *is, AVFrame *pFrame,AVFrame *pFrameRGB, double pts) {
		...
		// Convert the image into YUV format that SDL uses
	    	sws_scale(is->sws_ctxRGB,(const uint8_t * const *) pFrame->data,pFrame->linesize, 0, is->video_st->codec->height,pFrameRGB->data,
				pFrameRGB->linesize);
				SDL_LockMutex(global_mutex);
				switch(is->Fcolor){
				case 'R':
							
          		for (k=0; k<(vp->width)*(vp->height); ++k) {
		    ...
		    case 'G':
			       	for (k=0; k<(vp->height)*(vp->width); ++k) {
        ...
        if(is->Fcolor == 'W'){
						memset(pict.data[1],128, (is->video_st->codec->height*is->video_st->codec->width)/4);
						memset(pict.data[2],128, (is->video_st->codec->height*is->video_st->codec->width)/4);
			  }
...
int decode_interrupt_cb(void *opaque) {
				return (nChannel[Channel] && nChannel[Channel]->quit);
}
...
int main(int argc, char *argv[]) {
		...
		*nChannel = (VideoState*)av_mallocz(sizeof(argc-1));
		...
		for(i=0; i < argc-1; i++){
				if(argv[i+1] != NULL){				
						...
						nChannel[i] = av_mallocz(sizeof(VideoState));
						av_strlcpy(nChannel[i]->filename, argv[i+1], sizeof(nChannel[i]->filename));
						nChannel[i]->chan = i;
						nChannel[i]->Audio = i;
 						
						 av_strlcpy(nChannel[i]->filename,argv[i+1],sizeof(nChannel[i]->filename));

						nChannel[i]->pictq_mutex = SDL_CreateMutex();
						nChannel[i]->pictq_cond = SDL_CreateCond();

  					schedule_refresh(nChannel[i], 40);

						nChannel[i]->av_sync_type = DEFAULT_AV_SYNC_TYPE;

 						nChannel[i]->parse_tid = SDL_CreateThread(decode_thread,nChannel[i]);
						if(!nChannel[i]->parse_tid){
								av_free(nChannel[i]);
								puts("Out");
								return -1;
						}
		...
		for (;;) {
		  	...
		  	SDL_WaitEvent(&event);
				...
				case SDL_KEYDOWN:
						...
						case SDLK_r:
									printf("REd  %d\n",Channel);
									nChannel[Channel]->Fcolor = 'R';
									break;
						case SDLK_g:
									printf("Green  %d\n",Channel);
									nChannel[Channel]->Fcolor = 'G';
									break;
						case SDLK_b:
									printf("Blue  %d\n",Channel);
									nChannel[Channel]->Fcolor = 'B';
									break;
						case SDLK_w:
									printf("White  %d\n",Channel);
									nChannel[Channel]->Fcolor = 'W';
									break;
						case SDLK_c:
									nChannel[Channel]->Fcolor = 'C';
									puts("regular colors\n");
									break;
						default:
			...
			case SDL_QUIT:
						for(i=0;i<argc-1;i++){
						SDL_CondSignal(nChannel[Channel]->audioq.cond);
						SDL_CondSignal(nChannel[Channel]->videoq.cond);
						nChannel[Channel]->quit = 1;
						SDL_Quit();
						}
						exit(0);
						break;
